...
- p3: Schema normalization discussion: design a normalized schema (FK tables for checklist/section/procedure/action/spec) to reduce string duplication and define migration steps.
...
- p3: Checklist selector UX: add search/filter for large lists; rely on API to return distinct checklist names (extend API if needed).
